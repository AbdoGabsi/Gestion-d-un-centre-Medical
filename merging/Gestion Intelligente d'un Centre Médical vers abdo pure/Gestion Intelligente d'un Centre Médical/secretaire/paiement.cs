...
namespace Gestion_Intelligente_d_un_Centre_Médical.secretaire
{
    public partial class paiement : Form
    {
        ...
        private int numero_facture_payé;//utiliser pour generer le recu 
        ...
        private void paiement_Load(object sender, EventArgs e)
        {
            ...
            OleDbCommand cmd = new OleDbCommand(req2, DataBase.connection);
...
WHERE Paiement.numero_facture = f.numero_facture AND Paiement.numero_rendez_vous = r.numero_rendez_vous AND r.cin_patient = patient.cin_patient and patient.cin_patient='" + cin_patient + "' AND f.etat='impayé';";
        ...
        private void valider_payment()
        {
            ...
            DataBase.connection.Open();
            req = $"SELECT numero_facture,etat,montant FROM facture WHERE numero_facture={dataGridView1.SelectedRows[0].Cells[0].Value.ToString()};";
            adapter = new OleDbDataAdapter(req, DataBase.connection);
            ...
            numero_facture_payé = int.Parse(row[0].ToString()); //a utiliser pour generer le recu 
        ...
        private void payer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                recu recu = new recu(numero_facture_payé);
                recu.Show();
